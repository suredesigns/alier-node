...
class OracleDBConnector extends DBConnector {
    static IsolationLevel = IsolationLevel;

    /**
     * An OracleDB client pool
     * @type {(import("oracledb").Pool | Promise<import("oracledb").Pool>)?}
     */
    #pool = null;
    /**
     * An OracleDB client
    ...
    #client = null;
    /**
     * @type {import("oracledb").PoolAttributes}
     */
    #client_config;
    /**
     * A callback function invoked whenever a connection becomes available.
     * 
     * @type {((client: import("oracledb").Connection) => void)?}
     */
    #on_connect = null;
    /**
     ...
     * @constructor
     * 
     * Creates a new instance of {@link OracleDBConnector}.
     * 
     * @param {object} o 
     * An object containing configuration options.
     * 
     * This object is a compound of {@link https://node-oracledb.readthedocs.io/en/latest/api_manual/oracledb.html#oracledb.createPool|PoolAttributes} defined in node-oracledb
     ...
     * 
     * This is an alias for `ppolMax` of `PoolAttributes`.
     * 
     ...
     * 
     * @param {((client: import("oracledb").Connection) => void)?} o.onConnect
     * A callback function invoked whenever a connection becomes available.
     * 
     * This function is invoked when {@link connect()} method is 
     * invoked with the instance newly created.
     * 
     * @param {((client: import("oracledb").Connection) => void)?} o.onDisconnect
     * A callback function invoked whenever a connection is released.
     * 
    ...
    constructor(o) {
        ...
        const { usePool: use_pool, onConnect: on_connect, onDisconnect: on_disconnect, ...config } = o_;
        const use_pool_ = typeof use_pool === "boolean" ? use_pool : false;

        if (on_connect != null && typeof on_connect !== "function") {
        ...
        if (use_pool_) {
            ...
            if (typeof pool_max !== "number" || Number.isNaN(pool_max) || pool_max <= 0) {
                pool_max = max;
            }
     ...
     * connecting with the backend OracleDB server before releasing
     * connection pool.
     * 
     * @throws {DBError}
     * When
    ...
    async end() {
        ...
        this.#pool = null;
        if (pool != null) {
            const pool_ = (pool instanceof Promise) ? await pool : pool;
            try {
                await pool_.close();
            } catch(error) {
    ...
    async execute(statement, ...params) {
        ...
        try {
            ...
            const { rows: records } = await this.#client.execute(statement, params);
            return {
                status: true,
                records
            };
     ...
     * 
     * If the target `OracleDBConnector` uses connection pooling,
     * connection is established with the pooled client.
     * Otherwise, this function creates a new standalone client and then
     * tries to connect to the backend OracleDB server by using it.
     * 
    ...
    async connect() {
        ...
        try {
            ...
            const client = (pool == null) ?
                await oracledb.getConnection(this.#client_config) :
     ...
     * @throws {DBError}
     * When
     * -    the underlying client failed to disconnect from the OracleDB server
